...
const MAX_POLLINATORS = 10;
const FLY_SPEED = 26;
const SEEK_NEAR = 76; // px a lifted pollinator looks for its next bloom
const SPAWN_PATCH_R = 44; // px sampled around a probe point when one arrives
const SPARKLE_TIME = 0.7; // seconds the pollen whisper hangs in the air

// What a pollinator can work: flowers always, shrubs carry blossom among the
// berries, succulents only once the genome has sent up its bloom spike (the
// same rule the sprite uses). Trees, ferns, fungi and coral hold nothing to sip.
...
// Circular closeness of two hues: 1 = the very colour it loves, 0 = the far
// side of the wheel.
export function hueAffinity(a: number, b: number): number {
  const raw = Math.abs(a - b) % 1;
  return 1 - Math.min(raw, 1 - raw) * 2;
}

// The bloom this pollinator would choose near a point: the colour it favors,
// close by, a true blossom before a berry bush. Moths at night barely see
// colour at all — they steer by whatever glows.
export function bestBloom(
  ...
  for (const p of plants) {
    if (p === exclude || !isBloom(p)) continue;
    const g = p.genome;
    let score =
      hueAffinity(favHue, g.hue) * (night ? 0.5 : 1.5) - Math.hypot(p.x - x, p.y - y) / 48;
    if (g.form === PlantForm.Flower) score += 0.6;
    if (night) score += g.glow * 2.4;
    if (score > bestScore) {
      bestScore = score;
      best = p;
    }
...
// Where the blossom actually is on the sprite — the flower's crown, the
// shrub's berry line, the succulent's spike — so a settled wing rests on the
// bloom and not on the dirt beside it.
function bloomHead(p: Plant): { x: number; y: number } {
  const g = p.genome;
  if (g.form === PlantForm.Flower) return { x: p.x + g.lean * 3, y: p.y - (4 + g.height * 11) };
  if (g.form === PlantForm.Shrub) return { x: p.x, y: p.y - (3 + g.height * 7) };
  return { x: p.x + g.lean, y: p.y - (6 + g.height * 8) }; // the succulent's spike
}

interface Pollinator {
  id: number; // stable identity — hashes into its phase and favored hue
  x: number;
  y: number;
  target: Plant; // the bloom currently sought or worked
  sip: number; // >0: seconds left settled at the bloom head
  sparkle: number; // >0: seconds of pollen shimmer left after touching down
  visits: number; // blooms worked so far; salts each whisper of pollen
  favHue: number; // the colour this one keeps returning to
  phase: number;
}

// Butterflies by day, pale moths by night — and they truly work the flowers:
// seek one out, settle at the bloom, sip a while with wings slowly beating,
// shake loose a breath of pollen, lift, and choose the next. Each carries a
// favored hue (worn on its wings), so which butterflies gather on which
// blooms is a pattern the lens can find. Purely ambient — the flora sim
// pollinates on its own; this is that work made visible.
export class Pollinators {
  private flock: Pollinator[] = [];
  ...
  update(
    ...
    // out of view, or the bloom died out from under it — gone
    this.flock = this.flock.filter(
      (w) =>
        w.x > viewX - 40 &&
        w.x < viewX + viewW + 40 &&
        w.y > viewY - 40 &&
        w.y < viewY + viewH + 40 &&
        flora.all[w.target.idx] === w.target,
    );

    // one probe per frame; richer patches call more of them in, so a
    // flowery place hums while a lone bloom gets the odd visitor
    if (this.flock.length < MAX_POLLINATORS) {
      const sx = viewX + this.rng() * viewW;
      const sy = viewY + this.rng() * viewH;
      const blooms = flora.plantsNear(sx, sy, SPAWN_PATCH_R).filter(isBloom);
      if (blooms.length > 0 && this.rng() < (blooms.length / 8) ** 2) {
        const id = this.nextId++;
        const favHue = hash2d(id, 1, 0x9011);
        const target = bestBloom(blooms, sx, sy, favHue, night);
        if (target) {
          const head = bloomHead(target);
          this.flock.push({
            id,
            x: head.x + (this.rng() - 0.5) * 56,
            y: head.y - 10 - this.rng() * 22,
            target,
            sip: 0,
            sparkle: 0,
            visits: 0,
            favHue,
            phase: hash2d(id, 2, 0x9011) * 6.28,
          });
        }
      }
    }

    for (const w of this.flock) {
      if (w.sparkle > 0) w.sparkle -= dt;
      const head = bloomHead(w.target);
      if (w.sip > 0) {
        // settled: hold at the bloom with the faintest breath of hover
        w.sip -= dt;
        w.x = head.x + Math.sin(t * 2.1 + w.phase) * 0.6;
        w.y = head.y - 1 + Math.sin(t * 3.3 + w.phase * 2) * 0.4;
        if (w.sip <= 0) {
          // lift, and choose the next bloom — the colour it loves, close by;
          // if this flower stands alone, it stays and works it again
          const next = bestBloom(
            flora.plantsNear(w.x, w.y, SEEK_NEAR),
            w.x,
            w.y,
            w.favHue,
            night,
          ...
          if (next) w.target = next;
          else w.sip = 0.8 + hash2d(w.id, w.visits, 0x9013) * 1.2;
        }
      } else {
        const dx = head.x - w.x;
        const dy = head.y - 1 - w.y;
        const d = Math.hypot(dx, dy);
        if (d < 1.4) {
          // touch down: sip a while, and let a breath of pollen go
          w.visits++;
          w.sip = 1 + hash2d(w.id, w.visits, 0x9012) * 1.2;
          w.sparkle = SPARKLE_TIME;
        } else {
          const sp = FLY_SPEED * Math.min(1, 0.3 + d / 14); // ease into the landing
          w.x += (dx / d) * sp * dt + Math.sin(t * 5.3 + w.phase) * 9 * dt;
          w.y += (dy / d) * sp * dt + Math.cos(t * 4.1 + w.phase) * 7 * dt;
        }
      }
    }
  }
  ...
  draw(ctx: CanvasRenderingContext2D, camX: number, camY: number, darkness: number): void {
    const night = darkness > 0.4;
    const t = this.lastMs / 1000;
    for (const w of this.flock) {
      const x = Math.round(w.x - camX);
      const y = Math.round(w.y - camY);
      const wing = night ? "rgba(255, 248, 225, 0.92)" : hsl(w.favHue, 0.75, 0.62);
      const body = night ? "rgba(240, 230, 200, 0.9)" : "rgba(40, 30, 30, 0.9)";
      if (w.sip > 0) {
        // at rest on the bloom: wings open in slow beats, folding together
        // over the back between them
        ctx.fillStyle = wing;
        if (Math.sin(t * 6 + w.phase) > 0.3) {
        ...
        const flap = Math.sin(t * 17 + w.phase) > 0 ? 1 : 0;
        ctx.fillStyle = wing;
      ...
      // the whisper of pollination: a few grains shaken loose as it settles,
      // drifting up off the bloom and fading — gold by day, pale by lamplight
      if (w.sparkle > 0) {
        const f = w.sparkle / SPARKLE_TIME;
        const a = ((night ? 0.65 : 0.55) * f).toFixed(3);
        ctx.fillStyle = night ? `rgba(226, 248, 255, ${a})` : `rgba(255, 233, 150, ${a})`;
        for (let k = 0; k < 3; k++) {
          const h1 = hash2d(w.id, w.visits * 3 + k, 0x9014);
          const h2 = hash2d(w.visits * 3 + k, w.id, 0x9014);
          ctx.fillRect(
            x + Math.round((h1 - 0.5) * 5),
            y - 1 - Math.round((1 - f) * (2 + h2 * 3)),
            1,
            1,
          );
        }
